...
function Navigation() {
  ...
  const handleSearch = async (query) => {
    ...
    try {
      const response = await fetch(`https://marketpulse-2025-2.onrender.com/api/search?q=${encodeURIComponent(query)}`);
          ...
          <div className="md:hidden py-4 border-t border-gray-200">
            <div className="flex flex-col gap-3">
              ...
              <div className="px-4 pb-3 border-b border-gray-200">
                ...
                {showSearchResults && (
                  <div className="mt-3 bg-gray-50 border border-gray-200 rounded-lg shadow-lg max-h-60 overflow-y-auto">
                    {searchLoading ? (
                      <div className="p-4 text-center">
                        <div className="animate-spin h-4 w-4 border-2 border-green-500 border-t-transparent rounded-full mx-auto"></div>
                      </div>
                      ...
                      searchResults.map((result, idx) => (
                        <div
                          key={idx}
                          className="flex items-center px-3 py-2 hover:bg-gray-100 border-b border-gray-100 last:border-b-0"
                        >
                          <button
                            onClick={() => selectStock(result)}
                            className="flex-1 text-left btn-hover"
                          ...
                          <button
                            onClick={(e) => addStockToDashboard(result, e)}
                            className="ml-2 p-1.5 text-green-600 hover:text-green-700 hover:bg-green-50 rounded-lg transition-fast btn-hover"
                            title="Add to dashboard"
                          >
